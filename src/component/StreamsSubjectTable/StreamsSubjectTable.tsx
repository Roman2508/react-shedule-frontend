...
//
//
const splitSubjects = (el, subgroupsCount, subjectType, zeroing, newSubjects, currentSubject) => {
  const subjects = [];
  let updatedSubject;
  ...
  if (subgroupsCount !== null) {
    for (let s = 0; s < subgroupsCount; s++) {
      ...
      currentSubject[subjectType] = 0;
    }
    //
    // if (newSubjects) {
    //   const findedItem = newSubjects.find(
    //     f => f.name === el.name && f.semester === el.semester && f.subgroupNumber === null
    //   );
    //   if (!findedItem) {
    //     updatedSubject = { ...el, subgroupNumber: null, [subjectType]: 0 };
    //   }
    // }
  }

  return [...subjects];
  // return [...subjects, updatedSubject];
};
//
//
//
...
const result = () => {
  ...
  const subjectsList = subjects.map(el => {
    ...
    const currentSubject = { ...el };

    //
    for (let i = 0; i < subgroups.length; i++) {
      //
      if (el.name === subgroups[i].name && el.semester === subgroups[i].semester) {
        // Дисципліна ділиться на підгрупи
        ...
        newSubjects.push(
          splitSubjects(
            ...
            newSubjects.flat(2).filter(el => el !== undefined),
            currentSubject
          )
        );
        newSubjects.push(
  ...
  return [...subjectsList, ...newSubjects];
};
